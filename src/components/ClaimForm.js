...
class ClaimForm extends Component {
  ...
  canSave = (forFeedback, forReview) => {
    console.log("canSave", this.autoGenerateClaimCode)
    // if (!this.autoGenerateClaimCode && !this.state.claim.code) return false; // If autoGenerated set back to true
    ...
    if (
      ...
      return false;
    if (!!this.showPatientCondition && this.showPatientCondition == true && !this.state.claim.patientCondition)
      return false;
    ...
    if (
      ...
      (!this.state.claim.referralCode ||
        this.state.claim.referralCode == null ||
        this.state.claim.referralCode == undefined)
    ) {
      return false;
    }
    if (this.state.claim.services !== undefined) {
      if (this.props.forReview || this.state.isRestored) {
        if (
          this.state.claim.services.length &&
          this.state.claim.services.filter((s) => !this.canSaveDetail(s, "service")).length
        ) {
          return false;
        }
      } else {
        if (
          this.state.claim.services.length &&
          this.state.claim.services.filter((s) => !this.canSaveDetail(s, "service")).length - 1
        ) {
          return false;
        }
      }
  ...
  onEditedChanged = (claim) => {
    ...
    if (this.props.onEditedChanged) {
      this.props.onEditedChanged(claim);
    }
  };

  _save = (claim) => {
    if (
      this.attachmentRequiredForReferral &&
      (claim.attachmentsCount == 0 || claim.attachmentsCount == undefined) &&
      (claim.visitType == REFERRAL || claim.patientCondition == REFERRAL)
    ) {
  ...
  _saveReview = (claim) => {
    this.setState({ lockNew: true, isSaved: true }, () => this.props.save(claim));
  };
  ...
  render() {
    ...
    let readOnly =
      ...
      (!forReview &&
        !forFeedback &&
        claim.status !== STATUS_ENTERED &&
        claim.status !== STATUS_RETURNED_FROM_FACILITY &&
        claim.status !== STATUS_RETURNED_FROM_BRANCH) ||
    ...
    const editingProps = {
      ...
      save:
        !!save && this.state.claim.status !== STATUS_REJECTED && !readOnly
          ? forReview
            ? this._saveReview
            : this._save
          : null,
